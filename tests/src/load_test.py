...
class TestUser(FastHttpUser):
    ...
    def on_start(self):
        with self.client.post(f'{_URL}/login', json=self.data, verify=False) as response:
    ...
    def user(self):
        ...
        with self.client.get(f'{_URL}/user', verify=False, headers={'X-OTC': otc_header}, catch_response=True) as response:
